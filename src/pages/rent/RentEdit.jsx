...
const RentEdit = ({ token, setError }) => {
  ...
  useEffect(() => {
    ...
    const getRentById = async () => {
        try {
            const response = await axios.get(`http://127.0.0.1:8000/a1/rent/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                }
            })
            ...
            setNoCar(response.data.no_car);
            setDateBorrow(response.data.date_borrow);
            setDateReturn(response.data.date_return);
            setDownPayment(response.data.down_payment);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/a1/rent/${id}`,
        {
          user_id: userId,
          tenant,
          no_car: noCar,
