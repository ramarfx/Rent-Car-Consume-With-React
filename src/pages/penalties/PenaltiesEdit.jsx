...
const PenaltiesEdit = ({ token, setError }) => {
  ...
  useEffect(() => {
    ...
    const getPenaltiesById = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/a1/penalties/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/a1/penalties/${id}`,
        {
          user_id: userId,
          no_car: noCar,
          keterangan,
